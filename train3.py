...
from utils.plots2 import plot_losses, plot_metrics, plot_confusion_matrix
...
def train(data, age_in_months, apply_blur, apply_contrast, weights, num_epochs, batch_size, lr, unfreeze_layer, resume_checkpoint, save_folder_name):
    ...
    all_class_precisions, all_class_recalls = [], []
    ...
    for stage_idx, train_loader in enumerate(train_loaders):
        ...
        all_val_precisions.extend(val_precisions)
        all_val_recalls.extend(val_recalls)
        all_class_precisions.extend(class_precisions)
        all_class_recalls.extend(class_recalls)
    ...
    for i in range(num_classes):
        ...
        precision = all_class_precisions[best_epoch][i].item()
        recall = all_class_recalls[best_epoch][i].item()
